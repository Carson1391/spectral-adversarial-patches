#!/usr/bin/env python3
"""
v30 — Multi-stable class-head attack for wrong-ID / tracking failure.

No embedding geometry. Directly attack the 80 class logits:
  - Each view should produce a confident class prediction
  - Different views should produce DIFFERENT confident classes
  - Objectness stays high so box survives

This makes the tracker see a person-shaped blob that keeps changing
identity labels frame-to-frame, breaking re-ID consistency.
"""
...
def augment_view(patch, mask, background):
    ...
    return paste_patch(background, p, m, y0, x0), (y0, x0)
...
def multistable_class_loss(preds_list, anchor_mask, alpha=1.0, lam=2.0):
    """
    For each view, select anchors overlapping the patch, apply softmax over classes.
    Loss = alpha * mean entropy - lambda * mean variance across views.

    entropy: push each view toward SOME confident class
    variance: different views -> different confident classes
    """
    all_cls = []
    for preds in preds_list:
        cls_logits = preds[:, anchor_mask, 5:85]
        # softmax over classes -> proper distribution per anchor
        sm = F.softmax(cls_logits, dim=-1)
        # Average distribution over patch anchors per view
        avg_sm = sm.mean(dim=1)  # (1, 80)
        all_cls.append(avg_sm)

    stack = torch.stack(all_cls, dim=0).squeeze(1)  # (V, 80)

    # Entropy: low entropy = confident prediction
    p = stack.clamp(1e-6, 1.0)
    ent = -(p * torch.log(p)).sum(dim=-1).mean()

    # Variance: maximize across views
...
def obj_loss(preds_list, anchor_mask):
    objs = torch.stack([p[:, anchor_mask, 4].sigmoid() for p in preds_list], dim=0)
    return -objs.mean()
...
def train(epochs=800, lr=0.005, n_views=8, alpha=1.0, lam=2.0,
          w_obj=1.0, w_tv=0.5, w_nps=0.1, w_shape=500.0, area_limit=0.25,
          init='rings', out_dir='outputs_v30', max_imgs=50):
    ...
    print(f"\nV30 Multi-Stable Class-Head Attack")
    ...
    for ep in range(epochs):
        ...
        loss_ms, ms_stats = multistable_class_loss(preds_list, anchor_mask, alpha, lam)
        loss_o = obj_loss(preds_list, anchor_mask)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out', type=str, default='outputs_v30')
