#!/usr/bin/env python3
"""
v34 — Multi-Peak NMS Saturation via Local-Maximum Loss.

Instead of entropy or top-K global, use max-pooling to find local maxima
in each YOLO grid's person-confidence map. Maximize the sum of these
local peaks. This creates many spatially separated person detections that
survive NMS.
"""
...
def reshape_to_grid(pred, stride, grid):
    """
    pred: (1, N, 85) where N = grid*grid*3
    Returns (1, 3, H, W, 85) per scale.
    """
    B, N, D = pred.shape
    assert N == grid * grid * ANCHORS_PER_CELL
    # Order is anchor-major: (grid_y, grid_x, anchor)
    pred = pred.view(B, grid, grid, ANCHORS_PER_CELL, D)
    pred = pred.permute(0, 3, 1, 2, 4)  # (B, A, H, W, D)
    return pred


def local_peak_loss(pred, stride, grid, top_k_peaks=20):
    """
    Reshape predictions to grid, compute person-confidence map,
    find local maxima via maxpool, maximize sum of top-K peaks.
    """
    gpred = reshape_to_grid(pred, stride, grid)
    obj = gpred[..., 4].sigmoid()          # (B, A, H, W)
    cls = gpred[..., 5:85].sigmoid()       # (B, A, H, W, 80)
    person_conf = obj * cls[..., PERSON]   # (B, A, H, W)

    # Max over anchors per cell
    cell_conf, _ = person_conf.max(dim=1, keepdim=True)  # (B, 1, H, W)

    # Find local maxima with 3x3 maxpool
    local_max = F.max_pool2d(cell_conf, kernel_size=3, stride=1, padding=1)
    is_peak = (cell_conf == local_max).float()
    peak_vals = cell_conf * is_peak

    # Take top-K peaks
    flat = peak_vals.view(-1)
    if flat.numel() > top_k_peaks:
        vals, _ = flat.topk(top_k_peaks)
    else:
        vals = flat

    loss = -vals.mean()
    n_peaks = (peak_vals > 0.25).float().sum().item()
    return loss, {'peak': loss.item(), 'n_peaks': n_peaks, 'max_conf': cell_conf.max().item()}


def multi_scale_peak_loss(pred):
    total = torch.tensor(0.0, device=DEVICE)
    stats = {}
    offset = 0
    for stride, grid in zip(STRIDES, GRID_SIZES):
        n = grid * grid * ANCHORS_PER_CELL
        scale_pred = pred[:, offset:offset+n, :]
        offset += n
        loss, s = local_peak_loss(scale_pred, stride, grid, top_k_peaks=20)
        total = total + loss
        for k, v in s.items():
            stats[f'{k}_s{stride}'] = v
    return total, stats
...
def setup_csv(out_dir):
    ...
    with open(path, 'w', newline='') as f:
        csv.writer(f).writerow(['epoch','time','act_s8','act_s16','act_s32','peak_s8','peak_s16','peak_s32','npeaks_s8','npeaks_s16','npeaks_s32','tv','nps','shape','total'])
    return path


def train(epochs=800, lr=0.005, w_act=1.0, w_peak=3.0, w_tv=3.0, w_nps=0.1,
          w_shape=500.0, area_limit=0.25, init='chevrons', out_dir='outputs_v34'):
    ...
    print(f"\nV34 Local-Peak NMS Saturation")
    print(f"  epochs={epochs} lr={lr}")
    print(f"  w_act={w_act} w_peak={w_peak} w_tv={w_tv} w_nps={w_nps} w_shape={w_shape}\n")
    ...
    for ep in range(epochs):
        ...
        loss_peak, peak_stats = multi_scale_peak_loss(out)
        loss_peak = w_peak * loss_peak
        ...
        total = loss_act + loss_peak + loss_tv + loss_n + loss_shape
        ...
        stats = {
            ...
            'peak_s8': peak_stats.get('peak_s8', 0),
            'peak_s16': peak_stats.get('peak_s16', 0),
            'peak_s32': peak_stats.get('peak_s32', 0),
            'npeaks_s8': peak_stats.get('npeaks_s8', 0),
            'npeaks_s16': peak_stats.get('npeaks_s16', 0),
            'npeaks_s32': peak_stats.get('npeaks_s32', 0),
            'tv': loss_tv.item(),
            'nps': loss_n.item(),
        ...
        with open(csv_path, 'a', newline='') as f:
            csv.writer(f).writerow([ep, datetime.now().isoformat(),
                ...
                stats['peak_s8'], stats['peak_s16'], stats['peak_s32'],
                stats['npeaks_s8'], stats['npeaks_s16'], stats['npeaks_s32'],
        ...
        writer.add_scalar('Loss/peak_s8', stats['peak_s8'], ep)
        writer.add_scalar('Loss/peak_s16', stats['peak_s16'], ep)
        writer.add_scalar('Loss/peak_s32', stats['peak_s32'], ep)
        writer.add_scalar('Meta/npeaks_s8', stats['npeaks_s8'], ep)
        writer.add_scalar('Meta/npeaks_s16', stats['npeaks_s16'], ep)
        writer.add_scalar('Meta/npeaks_s32', stats['npeaks_s32'], ep)
        ...
        print(f"Ep {ep:4d}/{epochs} | tot={total.item():8.3f} | "
              f"s8={stats['act_s8']:4.2f} s16={stats['act_s16']:4.2f} s32={stats['act_s32']:4.2f} | "
              f"pks_s8={stats['npeaks_s8']:3.0f} pks_s16={stats['npeaks_s16']:3.0f} pks_s32={stats['npeaks_s32']:3.0f} | "
...
if __name__ == '__main__':
    ...
    parser.add_argument('--w_peak', type=float, default=3.0)
    ...
    parser.add_argument('--out', type=str, default='outputs_v34')
    args = parser.parse_args()

    train(args.epochs, args.lr, args.w_act, args.w_peak, args.w_tv, args.w_nps,
