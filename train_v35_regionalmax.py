#!/usr/bin/env python3
"""
v35 — NMS Saturation via Regional-Maxima Loss.

Divide each YOLO grid into coarse regions; maximize the max person-confidence
in each region. This creates spatially separated high-confidence person peaks
that survive NMS and choke the tracker.
"""
...
def reshape_to_grid(pred, grid):
    B, N, D = pred.shape
    pred = pred.view(B, grid, grid, ANCHORS_PER_CELL, D)
    pred = pred.permute(0, 3, 1, 2, 4)
    return pred


def regional_max_loss(pred, grid, n_regions=3):
    """
    Divide grid into n_regions x n_regions tiles; maximize max person-conf per tile.
    """
    gpred = reshape_to_grid(pred, grid)
    obj = gpred[..., 4].sigmoid()
    cls = gpred[..., 5:85].sigmoid()
    person_conf = obj * cls[..., PERSON]

    # Max over anchors per cell -> (B, 1, H, W)
    cell_conf, _ = person_conf.max(dim=1, keepdim=True)

    # Tile size must divide grid
    tile = grid // n_regions
    # MaxPool over non-overlapping tiles
    regional_max = F.max_pool2d(cell_conf, kernel_size=tile, stride=tile)

    # Maximize sum of regional maxima (push each region to have a strong peak)
    loss = -regional_max.mean()

    n_peaks = (regional_max > 0.25).float().sum().item()
    max_conf = regional_max.max().item()
    return loss, {'peak': loss.item(), 'n_peaks': n_peaks, 'max_conf': max_conf}


def multi_scale_regional_loss(pred):
    total = torch.tensor(0.0, device=DEVICE)
    stats = {}
    offset = 0
    for stride, grid in zip(STRIDES, GRID_SIZES):
        ...
        offset += n
        # Use fewer regions for smaller grids
        n_regions = 3 if grid >= 26 else 2
        loss, s = regional_max_loss(scale_pred, grid, n_regions=n_regions)
        total = total + loss
        for k, v in s.items():
...
def train(epochs=800, lr=0.005, w_act=1.0, w_peak=3.0, w_tv=3.0, w_nps=0.1,
          w_shape=500.0, area_limit=0.25, init='chevrons', out_dir='outputs_v35'):
    ...
    print(f"\nV35 Regional-Max NMS Saturation")
    ...
    for ep in range(epochs):
        ...
        loss_peak, peak_stats = multi_scale_regional_loss(out)
        ...
        print(f"Ep {ep:4d}/{epochs} | tot={total.item():8.3f} | "
              ...
              f"pks_s8={stats['npeaks_s8']:2.0f} pks_s16={stats['npeaks_s16']:2.0f} pks_s32={stats['npeaks_s32']:2.0f} | "
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out', type=str, default='outputs_v35')
