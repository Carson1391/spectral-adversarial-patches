#!/usr/bin/env python3
"""
v37 — Feature Density Saturation / "Fractal Human" Box Entropy Attack.

Based on the user's specification:
1. Maximize total volume of high-confidence Class 0 (person) predictions
2. Maximize spatial variance (scatter) of bounding box center coordinates (xc, yc)
3. Keep objectness high so the camera tries to process instead of ignoring
4. DAP triangle + TPS cloth deformation + heavy TV + NPS palette

Uses faithful Darknet YOLOv3 surrogate (85 channels: 4 box + 1 obj + 80 cls).
"""
...
def decode_predictions(pred):
    """
    pred: (1, 10647, 85)
    Returns per-scale tensors of shape (B, A, H, W, 85) and box centers.
    """
    B = pred.shape[0]
    outputs = []
    offset = 0
    for stride, grid in zip(STRIDES, GRID_SIZES):
        ...
        scale = scale.permute(0, 3, 1, 2, 4)  # (B, A, H, W, 85)
        outputs.append((scale, stride, grid))
        offset += n
    return outputs


def box_entropy_loss(pred, conf_thresh=0.25):
    """
    Loss = - (person confidence volume) - lambda * variance of box centers.
    We weight boxes by their person confidence and push centers apart.
    """
    scales = decode_predictions(pred)
    ...
    for scale, stride, grid in scales:
        ...
        th = scale[..., 3]
        obj = scale[..., 4].sigmoid()
        cls = scale[..., 5:85].sigmoid()
        person_cls = cls[..., PERSON]
        conf = obj * person_cls

        # Box centers in image coords (differentiable)
    ...
    # Volume: push many anchors to have high person confidence
    # Use soft top-K to avoid gradient saturation
    k = min(1000, conf.numel())
    top_conf, _ = conf.topk(k)
    volume_loss = -top_conf.mean()

    # Spatial scatter: weighted variance of centers
    # Higher conf = more influence. Encourage centers to spread across image.
    ...
    scatter_loss = -(var_cx + var_cy)

    # Also push average objectness up so camera doesn't ignore
    obj_loss = -obj.mean()

    total = volume_loss + 0.001 * scatter_loss + 0.5 * obj_loss
...
def save_preview(patch, mask, epoch, out_dir, stats):
    ...
    txt = f"e{epoch:04d} max={stats.get('max_conf',0):.3f} >0.5:{stats.get('conf_50',0):.0f} sc={stats.get('scatter',0):.1f}"
...
def setup_csv(out_dir):
    ...
    with open(path, 'w', newline='') as f:
        csv.writer(f).writerow(['epoch','time','volume','scatter','obj','max_conf','conf_50','conf_70','conf_90','tv','nps','shape','total'])
    return path


def train(epochs=800, lr=0.005, w_entropy=1.0, w_tv=3.0, w_nps=0.1,
          w_shape=500.0, area_limit=0.25, init='chevrons', out_dir='outputs_v37'):
    ...
    print(f"\nV37 Feature Density Saturation / Box Entropy")
    print(f"  epochs={epochs} lr={lr}")
    print(f"  w_entropy={w_entropy} w_tv={w_tv} w_nps={w_nps} w_shape={w_shape}\n")
    ...
    for ep in range(epochs):
        ...
        loss_entropy, ent_stats = box_entropy_loss(out)
        loss_entropy = w_entropy * loss_entropy
        ...
        total = loss_entropy + loss_tv + loss_n + loss_shape
        ...
        stats = {
            'volume': ent_stats['volume'],
            'scatter': ent_stats['scatter'],
            'obj': ent_stats['obj'],
            'max_conf': ent_stats['max_conf'],
            'conf_50': ent_stats['conf_50'],
            'conf_70': ent_stats['conf_70'],
            'conf_90': ent_stats['conf_90'],
            'tv': loss_tv.item(),
            'nps': loss_n.item(),
        ...
        with open(csv_path, 'a', newline='') as f:
            csv.writer(f).writerow([ep, datetime.now().isoformat(),
                stats['volume'], stats['scatter'], stats['obj'], stats['max_conf'],
        ...
        writer.add_scalar('Entropy/volume', stats['volume'], ep)
        writer.add_scalar('Entropy/scatter', stats['scatter'], ep)
        writer.add_scalar('Entropy/obj', stats['obj'], ep)
        writer.add_scalar('Entropy/max_conf', stats['max_conf'], ep)
        writer.add_scalar('Entropy/conf_50', stats['conf_50'], ep)
        writer.add_scalar('Entropy/conf_70', stats['conf_70'], ep)
        writer.add_scalar('Entropy/conf_90', stats['conf_90'], ep)
        ...
        print(f"Ep {ep:4d}/{epochs} | tot={total.item():8.3f} | "
              f"vol={stats['volume']:6.3f} | sc={stats['scatter']:8.1f} | "
              f"obj={stats['obj']:.3f} | max={stats['max_conf']:.3f} | "
...
if __name__ == '__main__':
    ...
    parser.add_argument('--w_entropy', type=float, default=1.0)
    ...
    parser.add_argument('--out', type=str, default='outputs_v37')
    args = parser.parse_args()

    train(args.epochs, args.lr, args.w_entropy, args.w_tv, args.w_nps,
