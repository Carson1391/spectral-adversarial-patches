#!/usr/bin/env python3
"""
v42 — Fractal Human with class purity + training on real person crops.

Builds on v41:
- White noise init
- FPN person-channel activation maximization at s8/s16/s32
- DAP triangle mask + TPS/EOT fabric deformation
- Lower TV=1.0 for rich texture

NEW:
- Class-purity loss: suppress non-person class confidences
- Train on real person torso crops (not grey canvas)
"""
import os, sys, math, argparse, csv, glob, random
...
def load_person_crops(dir_path, max_imgs=200):
...
def paste_onto_person(patch, mask, person_img, y0=96, x0=96):
    """Paste patch onto a person image. person_img: (3,416,416)"""
    canvas = person_img.clone()
...
def purity_loss(pred):
    """
    pred: (1, 10647, 85)
    Minimize the maximum confidence of all non-person classes.
    """
    cls = pred[:, :, 5:85].sigmoid()
    other = torch.cat([cls[:, :, :PERSON], cls[:, :, PERSON+1:]], dim=2)
    # Push top-k other classes down
...
def train(epochs=800, lr=0.01, w_act=1.0, w_purity=0.5, w_tv=1.0,
          w_shape=500.0, area_limit=0.25, topk=60, out_dir='outputs_v42'):
    ...
    crops = load_person_crops(person_dir, max_imgs=200)
    ...
    print(f"\nV42 Fractal Human + Purity + Person Crops")
    ...
    for ep in range(epochs):
        ...
        # Random person crop
        person_img = crops[random.randrange(len(crops))]
        canvas = paste_onto_person(aug_patch, aug_mask, person_img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out', type=str, default='outputs_v42')
