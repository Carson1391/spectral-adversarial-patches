#!/usr/bin/env python3
"""
v33 — Multi-Peak NMS Saturation Patch.

Goal: produce many overlapping high-confidence person detections that
choke the edge-node NMS/tracker.

Key changes from v32:
- Spatial diversity loss: push person confidence in many grid cells, not one.
- Anti-collapse penalty: entropy over spatial activation map prevents a single blob.
- Periodic/chevron initialization naturally creates multiple peaks.
- Multi-scale person activation still active.
- Heavy TV loss, TPS/EoT fabric simulation.
"""
...
def init_chevrons(size=224, n_chevrons=12):
...
def multi_peak_loss(preds, topk_per_scale=300, collapse_weight=1.0):
    """
    Maximize person confidence in many spatially separated anchors.
    - loss: mean of top-K person confidences (push many anchors high)
    - entropy penalty: prevent all activation collapsing to one anchor
    """
    obj_sig = preds[:, :, 4].sigmoid()
    cls_sig = preds[:, :, 5:85].sigmoid()
    person_conf = obj_sig * cls_sig[:, :, PERSON]

    # Top-K globally to push many anchors high
    vals = person_conf.view(-1)
    if vals.numel() > topk_per_scale:
        top_vals, _ = vals.topk(topk_per_scale)
    else:
        top_vals = vals
    loss_push = -top_vals.mean()

    # Entropy over the full distribution: spread activation across anchors
    p = person_conf.view(-1).clamp(1e-6, 1.0)
    p = p / p.sum()
    ent = -(p * torch.log(p)).sum()
    # We want HIGH entropy (spread), so loss = -entropy
    loss_spread = -collapse_weight * ent

    return loss_push + loss_spread, {'push': loss_push.item(), 'spread': loss_spread.item(), 'ent': ent.item()}
...
def save_preview(patch, mask, epoch, out_dir, stats):
    ...
    txt = f"e{epoch:04d} s8:{stats.get('act_s8',0):.2f} s16:{stats.get('act_s16',0):.2f} s32:{stats.get('act_s32',0):.2f} push:{stats.get('push',0):.3f} ent:{stats.get('ent',0):.2f}"
...
def setup_csv(out_dir):
    ...
    with open(path, 'w', newline='') as f:
        csv.writer(f).writerow(['epoch','time','act_s8','act_s16','act_s32','push','ent','tv','nps','shape','total'])
    return path


def train(epochs=800, lr=0.005, w_act=1.0, w_peak=2.0, w_tv=3.0, w_nps=0.1,
          w_shape=500.0, area_limit=0.25, init='chevrons', out_dir='outputs_v33'):
    ...
    for ep in range(epochs):
        ...
        loss_peak, peak_stats = multi_peak_loss(out, topk_per_scale=300, collapse_weight=1.0)
        ...
        with open(csv_path, 'a', newline='') as f:
            csv.writer(f).writerow([ep, datetime.now().isoformat(),
                stats['act_s8'], stats['act_s16'], stats['act_s32'], stats['push'], stats['ent'],
        ...
        writer.add_scalar('Loss/push', stats['push'], ep)
        writer.add_scalar('Loss/ent', stats['ent'], ep)
        ...
        print(f"Ep {ep:4d}/{epochs} | tot={total.item():8.3f} | "
              ...
              f"push={stats['push']:6.3f} | ent={stats['ent']:5.2f} | "
...
if __name__ == '__main__':
    ...
    parser.add_argument('--w_peak', type=float, default=2.0)
    ...
    parser.add_argument('--out', type=str, default='outputs_v33')
